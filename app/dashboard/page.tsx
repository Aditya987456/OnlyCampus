...
export default function DashboardHome() {
  return (
    <div className="flex min-h-full flex-col items-center justify-center py-10 text-center sm:py-16">

      {/* Hero Icon */}
      <div className="mb-5 flex h-14 w-14 items-center justify-center rounded-2xl bg-green-600 shadow-md sm:mb-6 sm:h-16 sm:w-16">
        <GraduationCap className="h-7 w-7 text-white sm:h-8 sm:w-8" strokeWidth={2} />
      </div>

      {/* Heading */}
      <h1 className="mb-2 px-2 text-2xl font-bold text-gray-800 sm:text-3xl">
        Welcome to <span className="text-green-600">OnlyCampus</span>
      </h1>
      <p className="mb-8 max-w-md px-2 text-xs text-gray-500 sm:mb-10 sm:text-sm">
        Your all-in-one campus portal. Select a channel from the sidebar or explore features below.
      </p>
      ...
      <div className="grid w-full max-w-2xl grid-cols-1 gap-3 sm:grid-cols-3 sm:gap-4">
        {cards.map(({ href, icon: Icon, title, desc, color }) => (
          <Link
            key={title}
            href={href}
            className={`group flex flex-col items-center gap-2 rounded-xl border p-4 transition-all duration-150 hover:shadow-md sm:gap-3 sm:p-6 ${color}`}
          >
            <div className={`w-11 h-11 rounded-xl flex items-center justify-center border ${color}`}>
              <Icon size={20} />
            </div>
